...
public class HttpClientFactory<R extends RequestEngineBuilder<R>> {
  ...
  private final RequestStrategy<R> requestStrategy;
  ...
  public HttpClientFactory(AsyncHttpClient http,
    ...
    this(http, hostsWithSession, contextSupplier, callbackExecutor, (RequestStrategy<R>) new DefaultRequestStrategy());
  }
   ...
   *          to execute
   */
  public HttpClient<R> with(Request request) {
   ...
   */
  HttpClientFactory<R> createCustomizedCopy(UnaryOperator<R> mapper) {
    return new HttpClientFactory<>(this.http, this.hostsWithSession, this.contextSupplier, this.callbackExecutor,
                                 this.requestStrategy.createCustomizedCopy(mapper),
                                 this.eventListeners);
  }
}
